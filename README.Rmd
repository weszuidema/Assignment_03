...
```{r}
y_hat <- predict(mod)
sigma <- sqrt(sum(residuals(mod) ^ 2) / mod$df.residual)
...
- Why can't you use F-tests to compare the different models in Table 1?
</div>
...
3. Calculate the F-statistic for each regression, and save them.
4. Repeat 1--3, 5000 times.

<div>
- Why are we calculating the F-statistic from the regression ... rather than ...?
- How would you calculate the p-value using that distribution?
</div>
